...
def clean_and_space_text(text):
    # Remove any Arun Yadav / AI Systems references
    text = re.sub(r'## 6\. AI Systems Engineering Connections.*', '', text, flags=re.DOTALL)
    text = re.sub(r'- \*\*AI Systems Engineering.*', '', text)
    text = re.sub(r'> \*\*कस्टमाइज़्ड फॉर:\*\*.*', '', text)
    
    # Make inline formulas display nicely with space around them
    # Replace inline equations in text that are key steps with displayed math
    text = re.sub(r'(\$\$[\s\S]*?\$\$)', r'\n\n\1\n\n', text)
    
    # Clean up double headers or multiple empty lines
    text = re.sub(r'\n{4,}', '\n\n', text)
    return text.strip()

def build_spacious_master_notes():
    ...
    doc += f"> **लक्ष्य:** UP Super TET, UPTET, KVS, DSSSB एवं अन्य प्रतियोगी परीक्षाओं के छात्रों के लिए विशेष गाइड\n"
    doc += f"> **विशेषता:** स्पष्ट एवं विस्तृत गणितीय सूत्र ($$ ... $$) | पवन सर की 10 शॉर्ट ट्रिक्स | विस्तृत चरणबद्ध उदाहरण\n\n"
    
    doc += "## विषय-सूची (Table of Contents)\n\n"
    doc += "1. [बीजगणितीय माइंडमैप एवं फ्लोचार्ट](#1-बीजगणितीय-माइंडमैप-एवं-फ्लोचार्ट)\n\n"
    doc += "2. [महत्वपूर्ण बीजगणित सूत्र (Master Formula Cheat Sheet)](#2-महत्वपूर्ण-बीजगणित-सूत्र-master-formula-cheat-sheet)\n\n"
...
```\n\n"""
    ...
    doc += "---\n\n"
    doc += "## 2. महत्वपूर्ण बीजगणित सूत्र (Master Formula Cheat Sheet)\n\n"
    doc += "> [!NOTE]\n"
    ...
    for idx, sf in enumerate(summaries, 1):
        ...
        cleaned = clean_and_space_text(content)
        doc += f"\n\n---\n\n"
        doc += cleaned + "\n\n"
    ...
    print(f"Spacious master notes written to {MASTER_FILE_PATH}!")

if __name__ == "__main__":
    build_spacious_master_notes()
